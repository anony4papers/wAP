...
def plot_curves_tri(gt_array_wAP, pd_array_wAP, gt_array_mAP, pd_array_mAP, gt_array_mIOU, pd_array_mIOU, roc_file=None, fpfn_file=None):
    plt.figure()
    fpr, tpr, ths = metrics.roc_curve(gt_array_wAP, pd_array_wAP)
    plt.plot(fpr, tpr)
    fpr, tpr, ths = metrics.roc_curve(gt_array_mAP, pd_array_mAP)
    plt.plot(fpr, tpr)
    fpr, tpr, ths = metrics.roc_curve(gt_array_mIOU, pd_array_mIOU)
    plt.plot(fpr, tpr)
    plt.title("RoC")
    plt.xlabel("fpr")
    plt.ylabel("tpr")   
    ...
    plt.plot(fpr_1, fnr)
    auc_mAP = metrics.auc(fpr_1, fnr)
    fpr_1, fnr, ths = fp_fn_curve(gt_array_mIOU, pd_array_mIOU)
    plt.plot(fpr_1, fnr)
    auc_mIOU = metrics.auc(fpr_1, fnr)
    plt.title("fp_fn curve")
    plt.xlabel("fpr")
    plt.ylabel("fnr")   
    plt.show()
...
def main(args):
    ...
    for line in result_f:
        ...
        mIOU_benign_list.append(1 - float(mIOU_benign))
        mIOU_adv_list.append(1 - float(mIOU_adv))
    ...
    plot_curves_tri(gt_wAP, pd_wAP, gt_mAP, pd_mAP, gt_mIOU, pd_mIOU, 'roc_{0}.png'.format(args.squeeze_type), 'fpfn_{0}.png'.format(args.squeeze_type))
